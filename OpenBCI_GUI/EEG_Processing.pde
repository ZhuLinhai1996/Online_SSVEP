...
class EEG_Processing_User {
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    //for example, you could loop over each EEG channel to do some sort of time-domain pr4ocessing 
    ...
    float alpha_peak_amplitude = -100;
    float alpha_peak_frequency = 0;
    ...
    for (int Ichan=0;Ichan < nchan; Ichan++) {
      ...
      for (int Ibin=0; Ibin < fftBuff[Ichan].specSize(); Ibin++){
        ...
        if(Ichan == 7)
        {
            if(FFT_freq_Hz<=11 && FFT_freq_Hz>=8)
            {
              if(FFT_value_uV > alpha_peak_amplitude)
              {
                alpha_peak_amplitude = FFT_value_uV;
                alpha_peak_frequency = FFT_freq_Hz;
              }
            }  
        }
        println("alpha_peak_amplitude = " + alpha_peak_amplitude + ", alpha_peak_frequency = " + alpha_peak_frequency);
        */
